...
func NewLinks() *Links {
	...
	for i := 0; i < len(catValues); i++ {
		for j := 0; j < len(cityValues); j++ {
			cat := catValues[i]
			city := cityValues[j]
			job := "http://hydraruzxpnew4af.onion/catalog/" + cat + "?query=&region_id=" + city + "&subregion_id=0&price%5Bmin%5D=&price%5Bmax%5D=&unit=g&weight%5Bmin%5D=&weight%5Bmax%5D=&type=momental"
			jobs[links.i] = job
